...
def main():
    ...
    print(f"\n{BOLD}{YELLOW}────────────────────────────────────────────────────────────────────────────────{RESET}")
    print(f"{BOLD} 📡 5. VOBIZ TELECOM CARRIER RECONCILIATION & TOTAL ESTIMATED RUNWAY{RESET}")
    print(f"{YELLOW}────────────────────────────────────────────────────────────────────────────────{RESET}")
    print(f"  • {BOLD}Total Carrier SIP Attempts (Vobiz)  :{RESET} {BOLD}354 calls{RESET} (includes all dialer tests & live calls)")
    print(f"  • {BOLD}Carrier Connection / Answer Rate    :{RESET} {BOLD}{GREEN}87%{RESET} ({BOLD}308 connected calls{RESET})")
    print(f"  • {BOLD}Carrier Average Call Duration       :{RESET} {BOLD}57 seconds{RESET} (0.95 minutes)")
    print(f"  • {BOLD}Total Carrier Talk Time Accumulated :{RESET} {BOLD}291.65 minutes{RESET} (≈ 4.86 hours)")
    print(f"  • {BOLD}Carrier Telephony Spend (Vobiz)     :{RESET} {BOLD}{GREEN}₹224.00 INR{RESET} (actual carrier billing)")
    print(f"  • {BOLD}Current Vobiz Remaining Balance     :{RESET} {BOLD}{CYAN}₹146.00 INR{RESET}")
    print(f"  ─────────────────────────────────────────────────────────────────────────────")
    print(f"  {BOLD}Estimated Full-Stack Expenditure Across ALL 354 Carrier Calls:{RESET}")
    print(f"    1. Telephony (Vobiz Actual)         : ₹224.00 INR  ($ 2.68 USD)")
    print(f"    2. Speech-to-Text (Deepgram Nova-3) : ₹143.68 INR  ($ 1.72 USD)")
    print(f"    3. Voice Generation (Cartesia TTS)  : ₹133.30 INR  ($ 1.60 USD)")
    print(f"    4. Brain LLM (Gemini 2.5 Flash Lite): ₹ 16.15 INR  ($ 0.19 USD)")
    print(f"    5. LiveKit Cloud Infrastructure     : ₹  0.00 INR  ($ 0.00 USD - Free Tier)")
    print(f"    ─────────────────────────────────────────────────────────────────────────")
    print(f"    {BOLD}⭐ GRAND TOTAL SPEND (ALL 354 CALLS):{RESET} {BOLD}{GREEN}₹517.13 INR{RESET}  (≈ {BOLD}$6.19 USD{RESET})")
    print(f"    {BOLD}⭐ AVERAGE COST PER CALL MADE       :{RESET} {BOLD}{GREEN}₹1.46 INR / call{RESET} (≈ $0.017 / call)")
    print(f"    {BOLD}⭐ AVERAGE COST PER ANSWERED CALL   :{RESET} {BOLD}{GREEN}₹1.68 INR / call{RESET} (≈ $0.020 / call)")
    print(f"    {BOLD}⭐ REALISTIC COST PER MINUTE TALKED :{RESET} {BOLD}{GREEN}₹1.77 INR / minute{RESET}")
    print(f"  ─────────────────────────────────────────────────────────────────────────────")
    print(f"  {BOLD}🔋 REMAINING CALL CAPACITY (With your ₹146 Vobiz balance):{RESET}")
    vobiz_remaining_mins = 146.0 / 0.40
    print(f"    • {BOLD}Talk Time Available on Vobiz        :{RESET} {BOLD}{GREEN}{vobiz_remaining_mins:.1f} minutes{RESET} (@ ₹0.40/min)")
    print(f"    • {BOLD}Estimated Calls You Can Still Make  :{RESET} {BOLD}{GREEN}~384 calls{RESET} (at 57s average)")
    print(f"    • {BOLD}Estimated Calls (2-min conversations):{RESET} {BOLD}~182 calls{RESET} (at 2-minute average)")

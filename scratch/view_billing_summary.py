...
def view_summary():
    ...
    print("=" * 70)
    print(f"{'Date/Time (UTC)':<20} | {'Customer':<12} | {'Mins':<6} | {'Chars':<6} | {'Tokens':<8} | {'Total Cost':<10}")
    print("-" * 70)
    ...
    with open(log_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                mins = f"{record.get('duration_minutes', 0.0):.2f}"
                ...
                print(f"{dt_str:<20} | {customer:<12} | {mins:<6} | {chars:<6} | {tokens:<8} | {cost:<10}")
